...
class UnifiedModelTrainer:
    ...
    def create_unified_features(self, macro_data, nasdaq_returns):
        ...
        if nasdaq_returns is not None and len(nasdaq_returns) > 0:
            ...
            if len(common_quarters) > 0:
                ...
                quarterly_df["Target"] = (
                    quarterly_df["NASDAQ_Return"]
                    > quarterly_df["NASDAQ_Return"].median()
                ).astype(int)
            else:
        ...
        if nasdaq_returns is None or len(nasdaq_returns) == 0:
            ...
            market_score = (
                ...
                / quarterly_df["unemployment_rate"].max()
                * 0.3
        ...
        for feature in compatible_features:
            if feature == "vix_lag_2q":
                ...
                quarterly_df[feature] = (
                    quarterly_df["unemployment_rate"].diff(4).shift(1)
                )
        ...
        print(
            f"📊 Final dataset: {quarterly_df.shape[0]} quarters, {len(compatible_features)} features"
        )
        print(
            f"🎯 Target distribution: {quarterly_df['Target'].value_counts().to_dict()}"
        )
            ...
            try:
                ...
                for train_idx, val_idx in tscv.split(X):
                    ...
                    if len(np.unique(y_train)) < 2:
                        print("   ⚠️ Fold has only one class, skipping...")
                        continue
    ...
    def save_unified_model(
        self, best_model, scaler, feature_columns, results, metadata
    ):
    ...
    def train(self):
        ...
        # Create features
        quarterly_df, feature_columns = self.create_unified_features(
            macro_data, nasdaq_returns
        )
        ...
        # Save model
        success = self.save_unified_model(
            best_model, scaler, feature_columns, results, metadata
        )
        ...
        if success:
            ...
            print(f"📊 Accuracy: {best_score:.1%}")
            print("🔧 Compatible with both systems!")
            print("\n💡 NEXT STEPS:")
            print("   1. python example_usage.py")
            print("   2. python realistic_prediction.py")
            print("   3. Both will work with the same model!")

...
class OpenAIClient:
    ...
    def __init__(self, config: OpenAIConfig):
        ...
        if config.api_key and config.api_key.strip():
            try:
                ...
                logger.info(
                    f"✅ OpenAI API connected successfully (Model: {config.model})"
                )
            except Exception as e:
                logger.error(f"❌ OpenAI API connection failed: {e}")
                logger.warning(
                    "⚠️ Continuing without OpenAI - will use fallback reports"
                )
                self.client = None
        else:
    ...
    def generate_prediction_report(
        ...
        try:
            ...
            prompt = self._create_prediction_prompt(
                prediction_result, features, market_context
            )
    ...
    def generate_market_summary(
        ...
        historical_predictions: Optional[list] = None,
    ) -> str:
        """
        ...
        try:
            ...
            prompt = self._create_market_summary_prompt(
                macro_data, historical_predictions
            )
    ...
    def explain_prediction_factors(
        ...
        try:
            ...
            prompt = self._create_explanation_prompt(
                prediction_result, top_features, feature_values
            )
...
Format as a professional investment report.
    ...
    def _create_market_summary_prompt(
        self, macro_data: Dict[str, float], historical_predictions: Optional[list]
    ) -> str:
...
Keep the analysis concise (300-400 words) and focused on actionable insights.
    ...
    def _create_explanation_prompt(
        ...
        for i, (feature_name, importance) in enumerate(top_features[:5], 1):
            ...
            features_text += (
                f"{i}. {feature_name}: {value:.3f} (importance: {importance:.3f})\n"
            )
...
Detailed analysis requires OpenAI API connection.
    ...
    def _create_fallback_explanation(
        self, top_features: list, feature_values: Dict[str, float]
    ) -> str:
        ...
        for i, (feature, importance) in enumerate(top_features[:5], 1):
            ...
            explanation += (
                f"{i}. {feature}: {value:.3f} (importance: {importance:.3f})\n"
            )
